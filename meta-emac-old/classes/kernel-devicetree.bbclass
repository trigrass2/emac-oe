		...
		for type in ${KERNEL_IMAGETYPE_FOR_MAKE}; do
			if [ "$type" = "zImage" ] && [ "${KERNEL_DEVICETREE_BUNDLE}" = "1" ]; then
				cat ${D}/${KERNEL_IMAGEDEST}/$type-${KERNEL_VERSION} \
					...
					> ${D}/${KERNEL_IMAGEDEST}/$type-${KERNEL_VERSION}-$dtb_base_name.$dtb_ext${KERNEL_DTB_BIN_EXT}
				ln -sf $type-${KERNEL_VERSION}-$dtb_base_name.$dtb_ext${KERNEL_DTB_BIN_EXT} \
					${D}/${KERNEL_IMAGEDEST}/$type-$dtb_base_name.$dtb_ext${KERNEL_DTB_BIN_EXT}
				if [ -e "${KERNEL_OUTPUT_DIR}/${type}.initramfs" ]; then
					cat ${KERNEL_OUTPUT_DIR}/${type}.initramfs \
						${D}/${KERNEL_IMAGEDEST}/$dtb_base_name.$dtb_ext \
						>  ${D}/${KERNEL_IMAGEDEST}/${type}-${INITRAMFS_NAME}-$dtb_base_name.$dtb_ext${KERNEL_DTB_BIN_EXT}
				fi
			fi
		done
	done
...
do_deploy:append() {
	for dtbf in ${KERNEL_DEVICETREE}; do
		...
		for type in ${KERNEL_IMAGETYPE_FOR_MAKE}; do
			if [ "$type" = "zImage" ] && [ "${KERNEL_DEVICETREE_BUNDLE}" = "1" ]; then
				install -m 0644 ${D}/${KERNEL_IMAGEDEST}/$type-${KERNEL_VERSION}-$dtb_base_name.$dtb_ext${KERNEL_DTB_BIN_EXT} \
				...
				if [ -e "${KERNEL_OUTPUT_DIR}/${type}.initramfs" ]; then
					install -m 0644 ${D}/${KERNEL_IMAGEDEST}/${type}-${INITRAMFS_NAME}-$dtb_base_name.$dtb_ext${KERNEL_DTB_BIN_EXT} \
